...
import type { CorridorDefinition, DashboardDataMap, TransportMode } from "@/types";
...
export function CorridorTabs({
  ...
  const validOriginsForDestination = Array.from(
    new Set(
      corridors
        .filter((corridor) => corridor.destination === selectedCorridor.destination)
  ...
  const selectedItem = dataMap[selectedCorridor.id]?.inference;

  return (
    <div className="rounded-2xl border border-primary-muted bg-card/70 p-3 backdrop-blur-xl">
      ...
      <div className="space-y-3">
        <div>
          ...
          >
            ...
            <SelectContent className="border-primary-muted bg-slate-900 p-1.5">
              {origins.map((origin) => {
                const disabled = !validOriginsForDestination.includes(origin);
                return (
                  <SelectItem key={origin} value={origin} disabled={disabled} className="min-h-9 px-3 py-2">
                    {origin}
                  </SelectItem>
                );
              })}
